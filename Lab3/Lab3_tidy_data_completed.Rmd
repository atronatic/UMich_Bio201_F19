...
nrow(scfa_indv) # nrow tells us the number of rows in the dataframe
ncol(scfa_indv) # ncol tells us the number of columns in the dataframe
dim(scfa_indv) # dim tells us the dimension (rowsxcolums) in the dataframe
colnames(scfa_indv) # colnames returns the names of all the columns in the dataframe
rownames(scfa_indv) # rownames returns the names of all the rows in the dataframe
glimpse(scfa_indv) # glimpse shows a preview of the dataframe
str(scfa_indv) # str is like glimpse but shows a more limited strcture of the dataframe
```
...
scfa_indv_qc <- scfa_indv %>%
  ...
  filter(sample_wt_g >= .1 & sample_wt_g <= 1) %>%
  filter(notes != "frozen >24hrs") %>%
  mutate(sample_wt_kg = sample_wt_g / 1000) %>%
  mutate(acetate_mmol_kg = round((acetate_mm*.002 / sample_wt_kg), digits = 2))%>%
  mutate(butyrate_mmol_kg = round((butyrate_mm*.002 / sample_wt_kg), digits = 2))%>%
  mutate(propionate_mmol_kg = round((propionate_mm*.002 / sample_wt_kg), digits = 2))%>%
  mutate(scfa_total_mmol_kg = acetate_mmol_kg + butyrate_mmol_kg + propionate_mmol_kg)%>%
  select(-ht_m, -wt_kg, -sample_wt_kg)
...
scfa_wkly_qc <- read_excel("raw_data/SCFA_data.xlsx",
                        ...
                        na = c("", "NA", "-----")) %>%
  rename_all(tolower)%>%
  rename(acetate_mean_mm = acetate_mean, butyrate_mean_mm = butyrate_mean, propionate_mean_mm = propionate_mean)%>%
  rename(acetate_median_mm = acetate_median, butyrate_median_mm = butyrate_median, propionate_median_mm = propionate_median)%>%
  filter(study_week == "week1" | study_week == "week3")%>%
  filter(quantity_compliant == "yes")%>%
  mutate(scfa_total_mean_mm = acetate_mean_mm + butyrate_mean_mm + propionate_mean_mm)%>%
  mutate(scfa_total_median_mm = acetate_median_mm + butyrate_median_mm + propionate_median_mm)
