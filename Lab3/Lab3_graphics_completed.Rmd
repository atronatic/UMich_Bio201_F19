...
ggplot(data = scfa_qc, #call dataset
       mapping = aes(x = participant_id, 
                     y = butyrate_mean_mm)) + #assign data to axes
...
> ** The facetted figure has differently colored points for each week, as opposed to the all black plot of the non-facetted figure. Additionally, the x and y axes are labeled as a whole, as opposed to labeled twice for each week plot. Also, the week headers are formatted nicer than the simply printed out weeks in the non-facetted figure. ** 
...
plot3_6 <- scfa_qc %>%
  ...
  labs(x = "Individuals",
       y = "Total fecal butyrate (mmol/kg)") +
  theme(axis.text.x = element_blank(),
        legend.position = "none")
