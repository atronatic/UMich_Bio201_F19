...
```
###I would consider these samples to be unequal. The spread of the data is very different with sample1 having a much lower average and smaller spread than sample2.
### Example 2
```{r}
...
ggplot(data = df2, aes(x = samples, #plot data frame 
  ...
  theme(legend.position = "none")
```
### These two samples are more likely to be similar, but they are still fairly different. sample4 reaches into the higher y-axis values that sample3 does not.
### Example 3
```{r}
...
ggplot(data = df3, aes(x = samples, #plot data frame 
  ...
  theme(legend.position = "none")
```
### These samples are very different. For the most part, both samples are contained ito their own levels on the y-axis with very little overlap. Thus, their averages are fairly different and a good representation of their difference.
### Example 4
```{r}
...
ggplot(data = df4, aes(x = samples, #plot data frame 
  ...
  theme(legend.position = "none")
```
### These samples are the most similar in my opinion. Both samples have similar spreads, although sample8 has values that are sometimes above the highest values for sample7. Nonetheless, both samples are quite similar. 
# Import data 
...
```{r include=FALSE}
scfa_long <- read_delim("curated_data/SCFA_wkly_long.txt", 
...
scfa_wide <- read_delim("curated_data/scfa_wkly_wide.txt", 
    "\t", escape_double = FALSE, trim_ws = TRUE)%>%
  rename_all(tolower) %>%
  mutate(semester = factor(semester,
                           levels=c("Winter2015", "Fall2015", 
...
# The paired t-test resulted in a p-value of 0.2367. Thus, we cannot say with confidence that the butyrate concentrations between weeks 1 and 3 are not equal for individuals who consumed BRMPS once daily.
```
...
```{r}
# breath methane
methane_long <- read_delim("curated_data/breath_wkly.txt", 
    "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  rename_all(tolower)%>%
  filter(semester != "Winter2015",
         quantity_compliant == "yes") %>%
...
wk1_methane <- methane_long %>%
  filter(study_week == "week1") 
ggplot(wk1_methane, aes(x = study_week, y = ch4_mean)) + geom_violin() 

wk3_methane <- methane_long %>%
  filter(study_week == "week3")
ggplot(wk3_methane, aes(x = study_week, y = ch4_mean)) + geom_violin() 

```

```{r}
# acetate
acetate_long <- read_delim("curated_data/SCFA_wkly_long.txt", 
    "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  rename_all(tolower)%>%
  filter(semester != "Winter2015",
         quantity_compliant == "yes") %>%
...
wk1_acetate <- acetate_long %>%
  filter(study_week == "week1") 
ggplot(wk1_acetate, aes(x = study_week, y = acetate_mean)) + geom_violin()

wk3_acetate <- acetate_long %>%
  filter(study_week == "week3")
ggplot(wk3_acetate, aes(x = study_week, y = acetate_mean)) + geom_violin()
...
```{r}
# propionate
propionate_long <- read_delim("curated_data/SCFA_wkly_long.txt", 
    "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  rename_all(tolower)%>%
  filter(semester != "Winter2015",
         quantity_compliant == "yes") %>%
...
wk1_propionate <- propionate_long %>%
  filter(study_week == "week1") 
ggplot(wk1_propionate, aes(x = study_week, y = propionate_mean)) + geom_violin()

wk3_propionate <- propionate_long %>%
  filter(study_week == "week3")
ggplot(wk3_propionate, aes(x = study_week, y = propionate_mean)) + geom_violin()
```

```{r}
# pH
pH_long <- pH_wkly <- read_delim("curated_data/pH_wkly.txt", 
    "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  rename_all(tolower)%>%
  filter(semester != "Winter2015",
         quantity_compliant == "yes") %>%
...
wk1_ph <- pH_long %>%
  filter(study_week == "week1") 
ggplot(wk1_ph, aes(x = study_week, y = ph_mean)) + geom_violin()

wk3_ph <- pH_long %>%
  filter(study_week == "week3")
ggplot(wk3_ph, aes(x = study_week, y = ph_mean)) + geom_violin()
...
# breath methane
methane_long %>%
  filter(supplement_consumed == "BRMPS") %>% #subset long data frame in same manner as for plot
  group_by(frequency) %>% #set groups to what we are comparing
  summarise(sample_size = n()) #use summarise() and n() to get counts of ppl in each group
...
shapiro.test(met_df$ch4_mean) #call column of df with values = vector 
# code to extract first group (1xdaily)
once_grp_methane <- methane_long %>%
  filter(study_week == "week3",
         frequency == "1xdaily") 

# code to extract second group (2xdaily)
twice_grp_methane <- butyrate_long %>%
  filter(study_week == "week3",
         frequency == "2xdaily") 
```
...
shapiro.test(ace_df$acetate_mean) #call column of df with values = vector 
# code to extract first group (1xdaily)
once_grp_acetate <- acetate_long %>%
  filter(study_week == "week3",
         frequency == "1xdaily") 

# code to extract second group (2xdaily)
twice_grp_acetate <- acetate_long %>%
  filter(study_week == "week3",
         frequency == "2xdaily") 
```
...
pro_df <- propionate_long %>%
  filter(study_week == "week3") #subset long data frame in same manner as for plot

shapiro.test(pro_df$propionate_mean) #call column of df with values = vector 
# code to extract first group (1xdaily)
once_grp_propionate <- propionate_long %>%
  filter(study_week == "week3",
         frequency == "1xdaily") 

# code to extract second group (2xdaily)
twice_grp_propionate <- propionate_long %>%
  filter(study_week == "week3",
         frequency == "2xdaily") 
```
...
```{r}
# breath methane
join_met_df <- inner_join(x = wk1_methane, y = wk3_methane,
                    by = c("participant_id", "frequency", 
                           "semester", "supplement_consumed", "quantity_compliant")) %>%
...
t.test(x = join_met_df$methane_mean_wk1, y = join_met_df$methane_mean_wk3, 
       alternative = "less", paired = TRUE) 
```
# As the p-value is less than 0.05, we can conclude with confidence that there is a significant difference between the mean methane produced in week 3 and the mean methane produced in week 1. 
```{r}
# acetate
join_ace_df <- inner_join(x = wk1_acetate, y = wk3_acetate,
                    by = c("participant_id", "frequency", 
                           "semester", "supplement_consumed", "quantity_compliant")) %>%
...
t.test(x = join_ace_df$acetate_mean_wk1, y = join_ace_df$acetate_mean_wk3, 
       alternative = "less", paired = TRUE) 
```
# As the p-value is less than 0.05, we can conclude with confidence that there is a significant difference between the mean acetate produced in week 3 and the mean acetate produced in week 1.

```{r}
# propionate
join_pro_df <- inner_join(x = wk1_propionate, y = wk3_propionate,
                    by = c("participant_id", "frequency", 
                           "semester", "supplement_consumed", "quantity_compliant")) %>%
...
t.test(x = join_pro_df$propionate_mean_wk1, y = join_pro_df$propionate_mean_wk3, 
       alternative = "less", paired = TRUE) 
```
# As the p-value is greater than 0.05, we cannot conclude with confidence that there is a significant difference between the mean propionate produced in week 3 and the mean propionate produced in week 1. 
```{r}
# pH
join_ph_df <- inner_join(x = wk1_ph, y = wk3_ph,
                    by = c("participant_id", "frequency", 
                           "semester", "supplement_consumed", "quantity_compliant")) %>%
...
t.test(x = join_ph_df$ph_mean_wk1, y = join_ph_df$ph_mean_wk3, 
       alternative = "less", paired = TRUE) 
```
# As the p-value is greater than 0.05 (and in fact is almost 1), there is no significant difference between the pH values from week 1 and week 3. 
...
shapiro.test(s11$delta_butyrate) #p-value = 0.0654
...
a1 <- scfa_wide %>%
  filter(supplement_consumed == "Accessible") 
shapiro.test(a1$delta_acetate) #p-value = 0.674

a2 <- scfa_wide %>%
  filter(supplement_consumed == "Arabino") 
shapiro.test(a2$delta_acetate) #p-value = 0.0537

a3 <- scfa_wide %>%
  filter(supplement_consumed == "BRMPS+Accessible")
shapiro.test(a3$delta_acetate) #p-value = 0.8305

a4 <- scfa_wide %>%
  filter(supplement_consumed == "HiMaize") 
shapiro.test(a4$delta_acetate) #p-value = 0.0015

a5 <- scfa_wide %>%
  filter(supplement_consumed == "HiMaize+BRMPS") 
shapiro.test(a5$delta_acetate) #p-value = 0.3026

a6 <- scfa_wide %>%
  filter(supplement_consumed == "Inulin") 
shapiro.test(a6$delta_acetate) #p-value = 0.9105

a7 <- scfa_wide %>%
  filter(supplement_consumed == "LOODAT") 
shapiro.test(a7$delta_acetate) #p-value = 0.137

a8 <- scfa_wide %>%
  filter(supplement_consumed == "none")
shapiro.test(a8$delta_acetate) #p-value = 0.3468

a9 <- scfa_wide %>%
  filter(supplement_consumed == "Psyllium") 
shapiro.test(a9$delta_acetate) #p-value = 0.7713

a10 <- scfa_wide %>%
  filter(supplement_consumed == "Psyllium+BRMPS") 
shapiro.test(a10$delta_acetate) #p-value = 0.9725

a11 <- scfa_wide %>%
  filter(supplement_consumed == "transition_HiMaize") 
shapiro.test(a11$delta_acetate) #p-value = 0.6092

a12 <- scfa_wide %>%
  filter(supplement_consumed == "BRMPS") 
shapiro.test(a12$delta_acetate) #p-value = 0.0250
...
#Discussion
#I decided to conduct the ANOVA after determining that almost all of the supplement types were relatively normal. The ANOVA resulted in a p-value of 1.167e-5. As this is less than 0.05, the null hypothesis is rejected in favor of the alternative hypothesis that the different supplements produced statistically different amounts of acetate
...
p1 <- scfa_wide %>%
  filter(supplement_consumed == "Accessible") 
shapiro.test(p1$delta_propionate) #p-value = 6.51e-5

p2 <- scfa_wide %>%
  filter(supplement_consumed == "Arabino") 
shapiro.test(p2$delta_propionate) #p-value = NA - sample size too small (less than 3)

p3 <- scfa_wide %>%
  filter(supplement_consumed == "BRMPS+Accessible")
shapiro.test(p3$delta_propionate) #p-value = 0.2266

p4 <- scfa_wide %>%
  filter(supplement_consumed == "HiMaize") 
shapiro.test(p4$delta_propionate) #p-value = 0.2669

p5 <- scfa_wide %>%
  filter(supplement_consumed == "HiMaize+BRMPS") 
shapiro.test(p5$delta_propionate) #p-value = 0.4316

p6 <- scfa_wide %>%
  filter(supplement_consumed == "Inulin") 
shapiro.test(p6$delta_propionate) #p-value = 0.8973

p7 <- scfa_wide %>%
  filter(supplement_consumed == "LOODAT") 
shapiro.test(p7$delta_propionate) #p-value = 0.8389

p8 <- scfa_wide %>%
  filter(supplement_consumed == "none")
shapiro.test(p8$delta_propionate) #p-value = 0.1025

p9 <- scfa_wide %>%
  filter(supplement_consumed == "Psyllium") 
shapiro.test(p9$delta_propionate) #p-value = 0.1115

p10 <- scfa_wide %>%
  filter(supplement_consumed == "Psyllium+BRMPS") 
shapiro.test(p10$delta_propionate) #p-value = 0.6636

p11 <- scfa_wide %>%
  filter(supplement_consumed == "transition_HiMaize") 
shapiro.test(p11$delta_propionate) #p-value = 0.4711

p12 <- scfa_wide %>%
  filter(supplement_consumed == "BRMPS") 
shapiro.test(p12$delta_propionate) #p-value = 6.47e-6
...
# Discussion
# I decided to conduct the ANOVA test as most of the supplement types were normal, except for the Arabino supplement that only had 2 samples. Nonetheless, the ANOVA revealed a p-value of 0.006. As this p-value is less than 0.05, the null hypothesis is rejected in favor of the alternative hypothesis that the different supplements produced statistically different amounts of propionate.
```
