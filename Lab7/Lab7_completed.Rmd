...
richness_wk1 <- richness_df %>%
  ...
  select(study_week, avg_richness)
shapiro.test(richness_wk1$avg_richness) # p-value = 0.9016
group_by(richness_wk1) %>%
  summarize(sample_size = n()) # n = 97

richness_wk3 <- richness_df %>%
  filter(study_week == "week3") %>%
  select(study_week, avg_richness)
shapiro.test(richness_wk3$avg_richness) # p-value = 0.9783
group_by(richness_wk3) %>%
  summarize(sample_size = n()) # n = 88

# join dataframes
rich_join <- inner_join(x = richness_wk1, y = richness_wk3, by = c("participant_id", "semester")) %>%
  rename(richness_wk1 = avg_richness.x,
         richness_wk3 = avg_richness.y) 

# write code for paired t-test here 
t.test(x = rich_join$richness_wk1, y = rich_join$richness_wk3, alternative = "less", paired = TRUE)
# p-value = 1
```
...
```{r}
# check assumptions 
shannon_wk1 <- simpson_df %>%
  filter(study_week == "week1") %>%
  select(study_week, avg_richness)
shapiro.test(shannon_wk1$avg_richness) # p-value = 1.619e-10
group_by(shannon_wk1) %>%
  summarize(sample_size = n()) # n = 97

shannon_wk3 <- simpson_df %>%
  filter(study_week == "week3") %>%
  select(study_week, avg_richness)
shapiro.test(shannon_wk3$avg_richness) # p-value = 9.946e-12
group_by(shannon_wk3) %>%
  summarize(sample_size = n()) # n = 88
# join dataframes
shannon_join <- inner_join(x = shannon_wk1, y = shannon_wk3, by = c("participant_id", "semester")) %>%
  rename(shannon_wk1 = avg_richness.x,
         shannon_wk3 = avg_richness.y)
# write code for paired t-test here 
t.test(x = shannon_join$shannon_wk1, y = shannon_join$shannon_wk3, alternative = "less", paired = TRUE)
# p-value = 0.9576
```
